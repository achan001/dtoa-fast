...
#if USE_LIB
#define DTOA_HARD(c,m,b,x)  dtoa_hard(c,m,b,0)
#else   /* dtoa.c */
#define DTOA_HARD(c,m,b,x)  dtoa_hard(c,x,b)
static void dtoa_hard(struct dtoa_case *c, double x, int blen)
{
  char *s, *e;
  blen = sizeof(c->str)-1;
  s = dtoa_r(x, 2, c->len, &c->dec, &blen, &e, c->str+1, blen);
  c->len = e - s;
}
#endif

static int dtoa_round_trip(double x, int dec)
{
...
(int64_t m, double frac, int dec, double ulp, double x)
{
  ...
  if (edge.str[LAST]=='1' && frac-ulp <= i-1 &&
      dtoa_bot_shortest() && dtoa_round_trip(x, dec)) return &edge;

  if (safe.str[LAST]=='9' && frac+ulp >= j+1 &&
      dtoa_top_shortest() && dtoa_round_trip(x, dec)) return &edge;
...
char *dtoa_fast(double x, int digits, int *psgn, int *plen, int *pdec)
{
  ...
  *psgn = bexp >> 11;       // save sign
  ...
  if (!best->head) DTOA_HARD(best, m0, bexp0-BIAS-64, x);
