...
// By not follow printf specs, we can produce SHORTEST string
// mode 'gG'  : no unneeded pt, sign, 0, use exponent if shorter
// mode 'eErR': no unneeded pt, sign, 0's in exponent
...
char* dtoa_ifmt(char *s, int sgn, int len, int dec, char fmt)
{
  if (ISDIGIT(*s)) {                    // skip Inf, NaN
    int m = (len > 1);                  // multi digits mantissa
    if ((fmt|32)!='g' || dec < -2-m || dec-len > 2+m + (dec>10)) {
      char *p = &s[len];                // exponential form
      if ((fmt|32)=='r') dec -= len;    // pat = d+E-?d{1,3}
      else if (dec--, m) {s[-1]=s[0]; *s-- = '.';}
      *p++ = 'E' | (fmt&32);
      if (dec < 0) {*p++ = '-'; dec = -dec;}
      m = dec >= 10;                    // multi digits exponent
      if (dec >= 100) {*p++ = '0' + dec/100; dec %= 100;}
      if (m) *p++ = '0' + dec/10;
      *p++ = '0' + dec%10;
      *p = '\0';
