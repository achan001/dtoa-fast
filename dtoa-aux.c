...
static int dtoa_safe(int64_t hi, int gap, int dec)
{
  ...
  // convert hi = m2 BILLION + m1
  int m2 = lrint(1E-9 * (hi - (int64_t) (5E8-1)));
  ...
  for(;;) {
    int n = 8;
    do { safe.str[--i] = '0' + m1%10U; m1/=10U; } while (--n);
    safe.str[--i] = '0' + m1;
    if (m2 == 0) break;
    m1 = m2; m2 = 0;                // reuse code for billions
  }
  safe.str[0] = safe.str[j] - gap;  // low last digit
  while(safe.str[i] == '0') i++;    // trim front zeroes

  if (safe.str[0] <= '0')           // can shorten more
    while(safe.str[--j] == '0') ;
...
(int64_t m, double frac, int dec, double ulp)
{
  ...
  if (i != j) safe.str[LAST] -= j - (frac > 0.5);
  return &safe;
}
