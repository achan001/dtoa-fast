...
#define IEEE_8087
#endif
// setup: _control87(_PC_53 | _RC_NEAR , _MCW_PC | _MCW_RC);
//    ==  _control87(0x10000, 0x30300)
...
extern "C" char *dtoa_gay(double d, int mode, int ndigits,
...
#if !defined(NO_HEX_FP) || (!defined(NO_STRTOD_BIGCOMP) && defined(Honor_FLT_ROUNDS)) /*{*/
 static Bigint *
...
dtoa_r(double dd, int mode, int ndigits, int *decpt, int *sign, char **rve, char *buf, size_t blen)
{
 /*	Arguments ndigits, decpt, sign are similar to those
	...
	*/
	...
	U d2, eps;
	double ds;
	int ieps, ilim0, k0, k_check, try_quick=0;
#ifndef No_leftright
#ifdef IEEE_Arith
	U eps1;
#endif
...
#ifdef NO_LHINT         // ALBERT 10/29/17
    j = (315653 * be + 36117183) >> 20;
#else
	j = Lhint[be + 51];
#endif
	p10 = &pten[j];
 ...
 char *
dtoa_gay(double dd, int mode, int ndigits, int *decpt, int *sign, char **rve)
{
