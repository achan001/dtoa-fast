...
//
// Usage examples:
// gcc test.c -DTESTD -std=gnu99 -Id:/cpp -Ic:/ -DUSE_LIB=1 -lmapm
// gcc test.c -DTESTD -std=gnu99 -Id:/cpp -Ic:/ -DUSE_LIB=2 -lmapm
// gcc test.c -DTESTA -std=gnu99 -Id:/cpp -Ic:/ -DDIGITS=10 -DUSE_LIB=3
//            -DMODE=FE_TOWARDZERO
// gcc test.c -DTESTR -std=gnu99 -Id:/cpp -Ic:/ -DDIGITS=17 -DUSE_LIB=0
// gcc test.c -DTESTR -std=gnu99 -Id:/cpp -Ic:/ -DDIGITS=~0
//            -DMODE=FE_UPWARD -DUSE_LIB=4 -lgmp-10
//
...
// gcc test.c -DTESTR -DDUMP -std=gnu99 -Id:/cpp -Ic:/ -lgmp-10
//
...
#define Honor_FLT_ROUNDS
  ...
  #endif
  #if !GCC && USE_LIB      // USE_MAP==0, already have old dtoa.c
  #include "dtoa/dtoa.c"    // use the faster, newer, BF96 version
  #endif
  ...
  #elif GAY
    #if DIGITS >= 0         // dtoa.c does not supported -DIGITS
    #include "dtoa/dtoa.c"
    #include "dtoa/dtoa-ifmt.c"
...
#else
  #warning MUST define TESTA, TESTD, or TESTR
