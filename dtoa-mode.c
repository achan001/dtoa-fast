...
#if USE_LIB
#define DTOA_HARD(c,m,b,x,mode)  dtoa_hard(c,m,b,mode)
#else   /* dtoa.c */
#define DTOA_HARD(c,m,b,x,mode)  dtoa_hard(c,x,b,mode)
static void dtoa_hard(struct dtoa_case *c, double x, int blen, int mode)
{
  char *s, *e;
  blen = sizeof(c->str)-1;
  if (mode) fesetround(mode<0 ? FE_DOWNWARD : FE_UPWARD);
  s = dtoa_r(fabs(x), 2, c->len, &c->dec, &blen, &e, c->str+1, blen);
  if (mode) fesetround(FE_TONEAREST);
  c->len = e - s;
}
#endif

static int dtoa_round_trip(double x, int dec, int mode)
{
  char *s = edge.str + edge.head;
  s[0] = '.';
  sprintf(&s[edge.len + 1], "E%d", edge.dec = dec - edge.head);
  EPRINT("\nedge = %s\n" , s);
  if (mode) fesetround(mode<0 ? FE_DOWNWARD : FE_UPWARD);
...
(int64_t m, double frac, int dec, double ulp, double x, int mode)
{
  ...
  dtoa_safe(m + i, j - i);    // build safe case
  char bot = edge.str[LAST];  // save for mode 1

  if (edge.str[LAST]=='1' && frac-ulp <= i-1 &&
      dtoa_bot_shortest() && dtoa_round_trip(x, dec, mode)) return &edge;

  if (safe.str[LAST]=='9' && frac+ulp >= j+1 &&
      dtoa_top_shortest() && dtoa_round_trip(x, dec, mode)) return &edge;

  safe.dec = dec - safe.head; // safe is best
  if (safe.head + safe.len == LAST)
    switch (mode) {
      case 1: safe.str[LAST] = bot; break;
      case 0: safe.str[LAST] -= j - (frac > 0.5);
...
char *dtoa_fast(double x, int digits, int *psgn, int *plen, int *pdec)
{
  ...
  *psgn = bexp >> 11;       // save sign
  ...
  i = BIAS + 56 - bexp;     // hash lookup i, scale 10 ** i
  i = (i>>2) + (i>20) + !(0x1337e1337fULL & (1ULL<<i));
  MUL32(n2,n1,n0,bexp, t[i], t_bexp[i]); dec-=i;
  BIT96("mantissa only");
  ...
  if (!best->head) DTOA_HARD(best, m0, bexp0-BIAS-64, x, mode);
