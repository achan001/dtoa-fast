...
#include "float.h"        // get DBL_MAX
  ...
  #define FIX_ROUND(x)    FIX_ ## x()
  #define FIX_0x400()     round += ULP() - 1
  #define FIX_0x3ff()     round += ULP() + !!(n1&0x800)
  #define FIX_0x7ff()     round += ULP() + !round
#else
  #include "dtoa/dtoa.c"
  #define FIX_ROUND(x)    return strtod_gay(str, NULL)
#endif
  ...
  if (bot == 0) {
    switch (round) {
      case 0x400:   // round-nearest 0100 ...
        if ((n1 & 0xfff) == 0x400 && n0 == 0) FIX_ROUND(0x400);
        break;
      case 0x800:   // round-away-0  ?000 ...
        if ((n1 & 0x7ff) == 0x000 && n0 == 0) FIX_ROUND(0x400);
    }
  } else if (bot > HOLE) {
    switch (round) {
      case 0x400:   // round-nearest ?011 ...
        if ((n1 & 0x7ff) == 0x3ff && n0 > HOLE) FIX_ROUND(0x3ff);
        break;
      case 0:       // already reach DBL_MAX
        if (bexp==0x7fd && n2==~0u && n1>=~0x7ffu) break;
      default:      // to/away zero  ?111 ...
        if ((n1 & 0x7ff) == 0x7ff && n0 > HOLE) FIX_ROUND(0x7ff);
    }
  }
